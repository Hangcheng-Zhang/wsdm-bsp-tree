...
BSPTree::BSPTree(Mesh& p_meshA, Mesh& p_meshB, Tolerance& p_Tolerance, int p_MaxDepth, int p_LeafShresholdNum, BSPConstructType p_Type):
	m_Tolerance(p_Tolerance),m_Type(p_Type)
{
	vector<TriangleBSP> facesA;
	vector<TriangleBSP> facesB;
	...
	m_Header = new BSPTreeNode(facesA, facesB, m_Tolerance, 0, p_MaxDepth, p_LeafShresholdNum, true, p_Type);
...
void BSPTree::GetPartitionPlane(bspFaceRenderingInfo& r_Pplanes)
{
